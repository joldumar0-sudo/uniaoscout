...
import {
  ...
  PawPrint, ShieldAlert, LogOut, Loader2, UserCog, Bell, BellOff, Settings, Menu
} from "lucide-react";
  ...
  return (
    <div className="flex flex-col h-full bg-sidebar text-sidebar-foreground">
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        ...
        {isAdmin && (
          <>
            ...
            <Link to="/membros" onClick={onNavigate} className={cn(
            ...
            <Link to="/auditoria" onClick={onNavigate} className={cn(
              "flex items-center gap-3 px-3 py-2 rounded-md text-sm",
      ...
      <div className="border-t border-sidebar-border p-3 space-y-2">
        ...
        <Link to="/configuracoes" onClick={onNavigate} className={cn(
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Mobile top bar */}
        <header className="md:hidden sticky top-0 z-30 flex items-center gap-2 px-3 h-14 border-b bg-background/95 backdrop-blur">
          <SheetTriggerStandalone onClick={() => setOpen(true)} unread={unread} />
          <img src={logo} alt="ECM" className="h-8 w-8 rounded object-cover" />
          <div className="text-xs font-bold uppercase tracking-wide truncate">
            Escoteiros Católicos · MZ
          </div>
        </header>
...
function SheetTriggerStandalone({ onClick, unread }: { onClick: () => void; unread: number }) {
  return (
    <Button variant="ghost" size="icon" onClick={onClick} className="relative">
      <Menu className="h-5 w-5" />
      {unread > 0 && (
        <span className="absolute -top-0.5 -right-0.5 inline-flex items-center justify-center min-w-4 h-4 px-1 rounded-full bg-destructive text-destructive-foreground text-[9px] font-bold">
          {unread > 9 ? "9+" : unread}
        </span>
      )}
    </Button>
